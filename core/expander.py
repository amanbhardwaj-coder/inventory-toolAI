...
from core.normalizer import split_value, join_unique
...
def _apply_pricing(row, config):
    rules = config.get("business_rules", {}).get("pricing_rules", [])
    ...
    for rule in rules:
        if rule.get("type") == "metal_contains":
            if rule.get("contains", "").lower() in metal_value:
                row["Price"] = rule.get("price")
            elif "Price" not in row or not str(row.get("Price", "")).strip():
                row["Price"] = rule.get("default_price")
...
def expand_inventory(normalized_df, config=None):
    ...
    Each token creates a variant row.

    Existing original expander can replace this module later.
    """
    if config is None:
        config = {}
